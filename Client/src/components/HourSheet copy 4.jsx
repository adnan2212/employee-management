...
const HourSheet = ({ selectedDate }) => {
  ...
  const productionKPI = taskData.Production * 0.9375 || 0;
  const nonProductionKPI = taskData["Non-Production"] * 0.64375 || 0;
  const sumKPI = productionKPI + nonProductionKPI;
  const overAllKPI = parseFloat(sumKPI.toFixed(1));

  // Calculate the percentage for the CircularProgressbar
  const productionPercentage = (productionKPI / 0.9375 / 7.5) * 100;
  const nonProductionPercentage = (nonProductionKPI / 0.64375 / 5.5) * 100;
  const sum = productionPercentage + nonProductionPercentage;
  const sumText = parseFloat(sum.toFixed(2));

  console.log("Selected Date in HourSheet:", selectedDate);
  console.log("Task Data:", taskData);
  console.log("productionKPI Data:", productionKPI);
  console.log("nonProductionKPI Data:", nonProductionKPI);
  console.log("overAllKPI Data:", overAllKPI);
